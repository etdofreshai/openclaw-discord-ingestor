 ...
 * Jan 1 1970 was Thursday; Monday is 4 days later (Jan 5).
 * Used to anchor weekly and biweekly boundary alignment.
 */
 ...
 *
 * - Sub-day cadences (1m…12h): UTC arithmetic against Unix epoch midnight.
 * - 1d / 3d: midnight UTC, aligned to epoch.
 * - 1w: next Monday 00:00 UTC.
 * - 2w: next biweekly Monday 00:00 UTC (anchored to 1970-01-05).
 * - 1mo: 1st of the next calendar month, 00:00 UTC.
 * - 2mo: 1st of the next Jan/Mar/May/Jul/Sep/Nov, 00:00 UTC.
 * - 3mo: 1st of the next quarter (Jan/Apr/Jul/Oct), 00:00 UTC.
 * - 4mo: 1st of the next Jan/May/Sep, 00:00 UTC.
 * - 6mo: 1st of the next Jan/Jul, 00:00 UTC.
 * - 1y: Jan 1 of the next calendar year, 00:00 UTC.
 *
 * The returned time is always **strictly after** `now` —
 * even when `now` falls exactly on a boundary, the NEXT boundary is returned.
 *
...
export function computeNextBoundary(cadence: CadencePreset, now: Date = new Date()): Date {
  ...
  switch (cadence) {
    ...
    case '1mo': {
      // 1st of next calendar month, 00:00 UTC
      return new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() + 1, 1));
    }
    case '2mo': {
      // Periods: Jan(0) Mar(2) May(4) Jul(6) Sep(8) Nov(10)
    ...
    case '6mo': {
      // Half-years: Jan(0) Jul(6)
      const m = now.getUTCMonth();
      if (m < 6) return new Date(Date.UTC(now.getUTCFullYear(), 6, 1));   // Jul 1
      return new Date(Date.UTC(now.getUTCFullYear() + 1, 0, 1));           // Jan 1 next year
    }
    case '1y': {
      return new Date(Date.UTC(now.getUTCFullYear() + 1, 0, 1));
    }
    default: {
      // Fallback — should not be reached with proper typing
      const p = 60 * 60_000;
      return new Date(Math.ceil(msP / p) * p);
    }
  }
