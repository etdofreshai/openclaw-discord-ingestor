...
async function waitForCdp(timeoutMs = 15000): Promise<void> {
  ...
  while (Date.now() - start < timeoutMs) {
    try {
      await cdpRequest('GET', '/json/version');
      return;
    } catch {}
    await sleep(300);
  }
...
export async function ensureChromium(): Promise<void> {
  ...
  starting = (async () => {
    try {
      ...
      await waitForCdp();
      console.log('[discord-ingestor] Chromium started with CDP on port ' + CDP_PORT);
    } finally {
      starting = null;
    }
  })();
...
export async function cdpNewTab(url?: string): Promise<any> {
  ...
  // Fallback: list and return newest
...
export async function createDiscordLoginTab(): Promise<{ webSocketDebuggerUrl: string; id: string }> {
  ...
  // Navigate to Discord login via CDP WebSocket
